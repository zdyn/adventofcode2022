export const fns = {
  "Part 1": (input) => {
    ...
    return 1000 * (node.x + 1) + 4 * (node.y + 1) + {"R": 0, "D": 1, "L": 2, "U": 3}[face];
  },
...
const setFlatMap = (map) => {
  ...
  map.forMatrix((node, i, j) => {
    ...
    if (map[i - 1] == null || map[i - 1][j] == null) {
      node.U = {node: map[map.map((r) => r[j]).findLastIndex(valid)][j], face: "U"};
    } else {
      node.U = {node: map[i - 1][j], face: "U"};
    }
    if (map[i + 1] == null || map[i + 1][j] == null) {
      node.D = {node: map[map.map((r) => r[j]).findIndex(valid)][j], face: "D"};
    } else {
